_addon.author = 'Ivaar; subjob support by Aurum'
_addon.name = 'AutoCOR'
_addon.commands = {'cor'}
...
cor_status = texts.new("",settings.text,setting)
...
local display_box = function()
	...
	if not play or (play.main_job ~= 'COR' and play.sub_job ~= 'COR') then
		cor_status:hide()
		return
	end

	local str = 'AutoCOR [O%s]':format(actions and 'n' or 'ff')
	str = str .. '\nRoll 1 [%s]':format(settings.roll[1])
	if (play.main_job == 'COR') then
		str = str .. '\nRoll 2 [%s]':format(settings.roll[2])
	end
	str = str .. '\n AoE:'
	for slot in party_slots:it() do
...
windower.register_event('prerender',function ()
    ...
    if nexttime + del <= curtime then
		...
		local ismain = play.main_job == 'COR'
        local abil_recasts = windower.ffxi.get_ability_recasts()
        if buffs[16] or is_moving or not aoe_range() then return end
        if buffs[309] then
            if abil_recasts[198] and abil_recasts[198] == 0 and ismain then
                use_JA('/ja "Fold" <me>')
            end
            return
        end
        ...
        for x = 1,rollcnt do
            ...
            if not buffs[roll.buff] then
                if abil_recasts[193] == 0 then
                    if x == settings.crooked_cards and ismain and abil_recasts[96] and abil_recasts[96] == 0 then
            ...
            elseif buffs[308] and buffs[308] == roll.id and buffs[roll.buff] ~= roll.lucky and buffs[roll.buff] ~= 11 then
                if ismain and abil_recasts[197] and abil_recasts[197] == 0 and not buffs[357] and L{roll.lucky-1,10,roll.unlucky > 6 and roll.unlucky}:contains(buffs[roll.buff]) then
...
windower.register_event('addon command', function(...)
        ...
        if commands[2] and commands[3] then
            ...
            if roll and not settings.roll:find(roll.en) then
                ...
                for k,v in pairs(rolls) do
                    if v and (v.en:startswith(commands[3]) or v.job == commands[3]) then
						local oldidx = settings.roll:find(v.en)
						if oldidx then
							--if the new roll is already set elsewhere, then swap positions instead
							settings.roll[oldidx] = settings.roll[commands[2]]
    ...
    elseif commands[1] == 'aoe' and commands[2] then
		if commands[2] == "on" or commands[2] == "off" then
			local newval = true
			if commands[2] == "off" then
				newval = false
			end
			for i=1,5,1 do
				settings.aoe['p' .. i] = newval
			end
			if newval then
				windower.add_to_chat(207, 'Will now ensure all members are in AoE range.')
			else
				windower.add_to_chat(207, 'Ignoring all party members.')
			end
		else
